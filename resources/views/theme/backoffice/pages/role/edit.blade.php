...
@section('breadcrumbs')
    ...
    <li class="breadcrumb-item active" aria-current="page">Role: {{$role->id}}</li>
@endsection
@section('dropdown')
@endsection
    ...
    <div class="container">
        ...
        <div class="container d-inline-flex justify-content-center">
            <div class="row w-50 pt-2 ">
                <form class="row g-2" method="POST" action="{{route('backoffice.role.update',$role)}}">
                    @csrf
                    @method('put')
                    <div class="mb-3">
                        <label for="name" class="form-label">Nombre del rol</label>
                        <input type="text" class="form-control" id="name" name="name" placeholder="Escriba un rol" value="{{$role->name}}">
                        @error('name')
                        <span  >
